...
//Fall 2014
//Assignment: Lab04
//Description: This progam takes splitted Images, calculates the color values of neighboring 
//Images and stiches them in correct order
//*******************************************************************************************
#include <iostream>
#include "EasyBMP.cpp"
#include "EasyBMP.h"
...
int findNorthWest(int score[2][16][16]) {
  ...
  int highestscore;
  int highestscoreidx;
  int west = score[0][0][1];
  int north = score[1][1][0];
  int bestwest = 0;
  int cumulative [16];
  int sum = 0;
  ...
  for(int i =0; i<16; i++)
    { 
      for(int j=0; j<16; j++)
	{
	  if (i != j &&(west>score[0][i][j]))
	    west=score[0][i][j];
	   
	  if (i != j &&(north>score[1][j][i]))
	    north=score[1][j][i];
	}
      sum = west + north;
      cumulative[i]= sum;
  ...
  highestscore=cumulative[0];
  highestscoreidx=0;
  
  for(int j =1; j<16; j++)
    {
      if(highestscore< cumulative[j])
	{     
	  highestscore = cumulative[j];
	  highestscore= j;
	}
    }
  return highestscoreidx; //returns the northwest tile  
...
int findEastNeighbor(int score[2][16][16], int tile, bool remaining[16]) {
  ...
  long long int lowestScore = 999999999999; //Initializing lowestscore to a very high number
  int ImageNumber = -1;
  for (int i = 0; i < 16; i++)
    {
      if (tile == i)
  ...
  if (ImageNumber >= 0)
    remaining[ImageNumber] = false; //setting remaining Images off so they go through the loop again
  return ImageNumber;
}
...
int findSouthNeighbor(int score[2][16][16], int tile, bool remaining[16]) {
  ...
  long int lowestScore = 100000000000000;
  int ImageNumber = -2;
  for (int i = 0; i < 16; i++)
    {
      if (tile == i)
...
int main()
{
  ...
  OutImg.WriteToFile("FinalImage.bmp"); //Write stiched Images to file
  return 0;
}
