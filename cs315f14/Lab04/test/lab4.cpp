...
int findNorthWest(int score[2][16][16]) {
  ...
  int Maxscore;
  int Maxscoreindex;
  int minwest = score[0][0][1];
  int minorth = score[1][1][0];
  int bestwest = 0;
  int culmitive [16];
  int sum = 0;
  ...
  for(int i =0; i<16; i++)
    { 
      for(int j=0; j<16; j++)
	{
	  if (i != j &&(minwest>score[0][i][j]))
	    minwest=score[0][i][j];
	   
	  if (i != j &&(minorth>score[1][j][i]))
	    minorth=score[1][j][i];
	}
      sum = minwest + minorth;
      culmitive[i]= sum;
  ...
  Maxscore=culmitive[0];
  Maxscoreindex=0;

  for(int j =1; j<16; j++)
    {
      if(Maxscore< culmitive[j])
	{     
	  Maxscore = culmitive[j];
	  Maxscoreindex= j;
	}
    }
  // finds the North West tile by adding the best North and West scores for each tile and
